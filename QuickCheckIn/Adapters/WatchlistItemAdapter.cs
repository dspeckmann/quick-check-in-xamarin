...
{
    public class WatchlistItemAdapter : BaseAdapter<TraktWatchlistItem>
    {
        Context context;
        TraktWatchlistItem[] items;

        public WatchlistItemAdapter(Context context, TraktWatchlistItem[] items)
            : base()
        {
        ...
        public override TraktWatchlistItem this[int position] => items[position];
        ...
        public override View GetView(int position, View convertView, ViewGroup parent)
        {
            ...
            if (listItem.Type == TraktSyncItemType.Movie)
            {
                text1.Text = listItem.Movie.Title;
                text2.Text = listItem.Movie.Year.ToString();
            }
            else if (listItem.Type == TraktSyncItemType.Show)
            {
